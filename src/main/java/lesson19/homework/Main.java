...
import lesson19.homework.deadlock.FirstClass;
import lesson19.homework.deadlock.SecondClass;
...
//        Ответ: запускал 10 раз, всегда последним значением было 3000 (проверил даже с циклом на 10тыс.);
...
/*        5.2. Какой диапазон результатов переменной возвращает поток после выполнения
        ...
        (пример 2980-3000);*/
//        Ответ: результат не изменился, запускал 5 раз, всегда последним значением было 3000;
...
//        6. Создать пример своего Deadlock-а;
        Thread firstThreadForDeadlock = new Thread(FirstClass::tryToRunInFirstClass);
        Thread secondThreadForDeadlock = new Thread(SecondClass::tryToRunInSecondClass);
        firstThreadForDeadlock.start();
        secondThreadForDeadlock.start();
    }
}
